...
const layerGroups = {
  renewables: L.layerGroup(),
  demand: L.layerGroup(),
  optimised: L.layerGroup(),
  landPrices: L.layerGroup(),
  dottedRegion: L.layerGroup(),
};
  ...
  plant: (status) =>
    L.divIcon({
      ...
      html: `<div style="background-color: ${
        status === "Existing" ? "#22c55e" : "#f97316"
      };" class="h-6 w-6 rounded-full border-2 border-white shadow-md"></div>`,
      iconSize: [24, 24],
      iconAnchor: [12, 12],
    }),
  storage: (status) =>
    L.divIcon({
      className: "custom-div-icon",
      html: `<div style="background-color: ${
        status === "Existing" ? "#3b82f6" : "#a855f7"
      };" class="h-6 w-6 rounded-sm border-2 border-white shadow-md"></div>`,
      iconSize: [24, 24],
      iconAnchor: [12, 12],
    }),
  renewable: (type) =>
    L.divIcon({
      className: "custom-div-icon",
      html: `<div style="background-color: ${
        type === "Solar" ? "#facc15" : "#60a5fa"
      };" class="h-5 w-5 rounded-full"></div>`,
      iconSize: [20, 20],
      iconAnchor: [10, 10],
    }),
  optimised: () =>
...
const layerControlCheckboxes = {
  ...
  dottedRegion: layerGroups.dottedRegion,
};
...
for (const [id, group] of Object.entries(layerControlCheckboxes)) {
  ...
  checkbox.addEventListener("change", (e) =>
    e.target.checked ? map.addLayer(group) : map.removeLayer(group)
  );
}
